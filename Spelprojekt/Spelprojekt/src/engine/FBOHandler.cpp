...
const vec2 poisson16[] = vec2[](    // Semi random sampling positions
...
void main ()
{
	...
	//SSAO variables
	float depth0 = texture(depth, texcoord).t;
	float ambientOcclusion = 0;
	vec2 filterRadius = vec2(5, 5);
    ...
    for (int i = 0; i < sample_count; ++i)
    {
        ...
        float sampleDepth = texture(depth, texcoord).r;
        vec3 samplePos = vec3(Position0.x, Position0.y, Position0.z);
        vec3 sampleDir = normalize(samplePos - vec3(Position0.x, Position0.y, Position0.z));
 
        // angle between SURFACE-NORMAL and SAMPLE-DIRECTION (vector from SURFACE-POSITION to SAMPLE-POSITION)
        float NdotS = max(dot(vec3(normal0.x, normal0.y, normal0.z), sampleDir), 0);
        // distance between SURFACE-POSITION and SAMPLE-POSITION
        float VPdistSP = distance(vec3(Position0.x, Position0.y, Position0.z), samplePos);
	...
	lighted_scene = mySample * CalcSpotLight(gSpotLight, normal0.xyz) * ambientOcclusion;
}
)";
